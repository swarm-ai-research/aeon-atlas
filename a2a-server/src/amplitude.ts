 ...
 *
 * This server has no in-process LLM SDK call site — `runSkillAsync()` spawns
 * the `claude` CLI as a subprocess and parses its `--output-format json`
 * reply. There's no OpenAI/Anthropic client object for @amplitude/ai to
 * wrap, so each skill run is tracked manually as a one-turn agent session,
 * using the usage/cost fields the CLI itself reports.
 */
...
if (!apiKey) {
  process.stderr.write(
    "[aeon-a2a] AMPLITUDE_AI_API_KEY missing — agent analytics disabled\n"
  );
}
